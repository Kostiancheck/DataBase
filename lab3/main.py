...
query1 = '''SELECT
    happiness_region.region,
    round(avg(happiness_region.happiness_score), 3) AS "happines score"
FROM
    happiness_region
GROUP BY
    happiness_region.region
...
query2 = '''SELECT  round(happiness_region.happiness_score/ max.maximum*100, 4) as percent
FROM happiness_region 
     ,(SELECT MAX(happiness_region.happiness_score) as maximum
     FROM happiness_region) max
WHERE happiness_region.country = 'Ukraine'
'''
...
pie = go.Pie(labels=['Ukraine', 'rest'], values=[ukraine_happiness, rest],pull=[ 0.2, 0],
...
py.dashboard_ops.upload(my_dboard, 'Be happy v.2')
